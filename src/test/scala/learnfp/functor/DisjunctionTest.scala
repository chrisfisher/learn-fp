...
class DisjunctionTest extends WordSpecLike with Matchers {
  "either functor" should {
    "works with left" in {
      left[String, Int]("foo") map { x =>
        x + "bar"
      } shouldBe LeftDisjunction[String, Int]("foo")
    }

    "works with right" in {
      right[String, Int](10) map { x =>
        x + 20
      } shouldBe RightDisjunction[String, Int](30)
    }
