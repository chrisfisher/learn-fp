...
object Nested {
  implicit def functorInstance[F[_], G[_], A](implicit fF: Functor[F],
                                              fG: Functor[G]) =
    new Functor[({ type E[X] = Nested[F, G, X] })#E] {
      override def map[A, B](a: Nested[F, G, A])(fx: A => B): Nested[F, G, B] =
        Nested[F, G, B] {
          fF.map(a.value) { xg =>
            fG.map(xg) { x =>
              fx(x)
  ...
  implicit def applicativeInstance[F[_], G[_], A](
    ...
    new Applicative[({ type E[X] = Nested[F, G, X] })#E] {
      ...
      override def <*>[A, R](nfx: Nested[F, G, A => R])(
          na: Nested[F, G, A]): Nested[F, G, R] = {
        lazy val fxfg: F[G[A => R]] = nfx.value
        lazy val afg: F[G[A]] = na.value
        Nested({ (fx: G[A => R], a: G[A]) =>
          fx <*> a
        }.curried `<$>` fxfg `<*>` afg)
      }
    }
