...
object DisjunctionInstance {
  ...
  implicit def eitherInstance[L] =
    new Functor[({ type E[A] = Disjunction[L, A] })#E] {
      override def map[A, B](a: Disjunction[L, A])(
          fx: A => B): Disjunction[L, B] = ???
    }

  implicit def baseToFunctorOps[L, R, D[L, R] <: Disjunction[L, R]](
      disjunction: D[L, R])(
  ...
  implicit class DisjunctionFxOps[A, R](fx: A => R) {
    def `<$>`[L](a: Disjunction[L, A]): Disjunction[L, R] = a map fx
  }
}
